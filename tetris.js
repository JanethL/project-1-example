...
// we need to create a function to help us save all the pieces
function createBlocks(w, h){
    const blocks = []; // we set an empty array
...
function update(time = 0) { //drop the blocks using the requestAnimationFrame by getting time and defaulting to 0
    ...
    dropCounter += deltaTime; //we add drop count to the diff between prev frame and current frame 
    if (dropCounter > dropInterval){
        player.pos.y++;
        dropCounter = 0; // we reset to 0 so it can count again 
...
const field = createBlocks(12, 20);
...
function collide(field, player){
    ...
    const [b, p] = [player.blocks, player.pos]; 
    ...
    for(let y = 0; y < b.length; y++){
        ...
        for(let x = 0; x <b[y].length; x++){
            ...
            if(b[y][x] !==0 && //if the players blocks  y row and x column is not zero (they collide)
                (field[y + p.y] && // and if fields row exist // if it doesn't exist it will count as collision 
                field[y + p.y][x + p.x]) !== 0){ //if it exist we can grab the child and if they are not zero (they collide) so we return true
